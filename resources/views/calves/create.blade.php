...
@section('title', 'Record New Calf - Dairy Farm')
@section('page-title', 'Record New Calf')

@section('breadcrumbs')
<li class="breadcrumb-item"><a href="{{ route('dashboard') }}">Dashboard</a></li>
<li class="breadcrumb-item"><a href="{{ route('calves.index') }}">Calves</a></li>
<li class="breadcrumb-item active">New Calf</li>
@endsection
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-md-6">
                                ...
                                <select class="form-select @error('dam_id') is-invalid @enderror" 
                                    ...
                                    @foreach($dams as $dam)
                                    <option value="{{ $dam->id }}" 
                                        {{ (old('dam_id', $selectedDam?->id) == $dam->id) ? 'selected' : '' }}>
                            ...
                            <div class="col-12">
                                ...
                                <select class="form-select @error('breeding_record_id') is-invalid @enderror" 
                                    ...
                                    <option value="{{ $record->id }}" 
                                        {{ (old('breeding_record_id', $selectedBreedingRecord?->id) == $record->id) ? 'selected' : '' }}>
                                        Record #{{ $record->id }} - {{ $record->dam->name ?? $record->animal->name ?? 'Unknown' }} 
                                        (Bred: {{ $record->date_of_service?->format('M d, Y') ?? 'N/A' }}, 
                                         Expected: {{ $record->expected_calving_date?->format('M d, Y') ?? 'N/A' }})
                                    </option>
                                    @endforeach
                                </select>
                                <small class="form-text text-muted">
                            ...
                            <div class="col-md-4">
                                ...
                                <select class="form-select @error('birth_type') is-invalid @enderror" 
                                    ...
                                    <option value="triplet" {{ old('birth_type') == 'triplet' ? 'selected' : '' }}>Triplet</option>
                                </select>
                                @error('birth_type')
                                <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            ...
                            <div class="col-md-4">
                                ...
                                <select class="form-select @error('vaccination_status') is-invalid @enderror" 
                                    ...
                                    <option value="complete" {{ old('vaccination_status') == 'complete' ? 'selected' : '' }}>Complete</option>
                                </select>
                            ...
                            <div class="col-12">
                                <div class="form-check">
                                    <input class="form-check-input" 
                                           ...
                                           value="1"
                                           {{ old('requires_special_care') ? 'checked' : '' }}>
                                    <label class="form-check-label" for="requires_special_care">
                                        This calf requires special care
                                    </label>
                                </div>
                            </div>

                            <div class="col-12" id="specialCareNotes" style="{{ old('requires_special_care') ? '' : 'display: none;' }}">
                                ...
                                <textarea class="form-control @error('special_care_notes') is-invalid @enderror" 
                                          ...
                                          rows="2"
                                          placeholder="Describe the special care requirements...">{{ old('special_care_notes') }}</textarea>
                            ...
                            <div class="col-12 mt-4">
                                <div class="d-flex justify-content-between">
                                    ...
                                    <button type="submit" class="btn btn-success">
                                        <i class="fas fa-save me-1"></i> Record Calf
                                    </button>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="row">
                        <div class="col-md-4">
                            <div class="d-flex align-items-center mb-2">
                                ...
                                <small>Record calves within 24 hours of birth</small>
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="d-flex align-items-center mb-2">
                                <i class="fas fa-check-circle text-success me-2"></i>
                                <small>Weigh calves immediately after birth</small>
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="d-flex align-items-center mb-2">
                                <i class="fas fa-check-circle text-success me-2"></i>
                                <small>Administer colostrum within first 6 hours</small>
                            </div>
                        </div>
                    </div>
                </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('date_of_birth').value = new Date().toISOString().split('T')[0];
